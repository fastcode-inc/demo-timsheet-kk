...
import { IProject, ProjectService, ProjectDetailsComponent, ProjectListComponent, ProjectNewComponent } from '../';
...
describe('ProjectListComponent', () => {
  ...
  let constData: IProject[] = [
    ...
    },
    {
      ...
      startdate: d,
      customerid: 2,
      customerDescriptiveField: 'name2',
    },
  ];
  let data: IProject[] = [...constData];

  describe('Unit tests', () => {
    beforeEach(async(() => {
      TestBed.configureTestingModule({
        declarations: [ProjectListComponent, ListComponent],
        imports: [TestingModule],
        providers: [ProjectService, ChangeDetectorRef],
  ...
  describe('Integration tests', () => {
    ...
    it('should verify that redirected to details page when edit button is clicked', async () => {
      ...
      expect(decodeURIComponent(location.path())).toBe(
        `/project/${ServiceUtils.encodeIdByObject(data[0], component.primaryKeys)}`
      );
    });
