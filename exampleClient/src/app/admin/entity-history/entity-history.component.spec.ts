...
import { IUsers } from 'src/app/admin/user-management/users/index';
...
describe('EntityHistoryComponent', () => {
  ...
  it('should get the list of associated parent', async () => {
    ...
    spyOn(component.usersService, 'getAll').and.returnValue(of(userData));
  ...
  it('should show error while fetching the list of users', async () => {
    ...
    spyOn(component.usersService, 'getAll').and.returnValue(
  ...
  it('should not call getAll method of user service if first condition is not true', async () => {
    ...
    spyOn(component.usersService, 'getAll').and.returnValue(of(null));
    component.onPickerScroll();

    expect(component.usersService.getAll).toHaveBeenCalledTimes(0);
  });
