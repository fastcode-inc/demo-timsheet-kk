...
public class TimesheetdetailsAppService implements ITimesheetdetailsAppService {
    ...
    public void delete(Long timesheetdetailsId) {
        Timesheetdetails existing = _timesheetdetailsRepository.findById(timesheetdetailsId).orElse(null);
    ...
    protected BooleanBuilder searchKeyValuePair(
        ...
        for (Map.Entry<String, SearchFields> details : map.entrySet()) {
            ...
            if (details.getKey().replace("%20", "").trim().equals("task")) {
                if (details.getValue().getOperator().equals("contains")) {
                    builder.and(
                        timesheetdetails.task.name.likeIgnoreCase("%" + details.getValue().getSearchValue() + "%")
                    );
                } else if (details.getValue().getOperator().equals("equals")) {
            ...
            if (details.getKey().replace("%20", "").trim().equals("timeofftype")) {
                if (details.getValue().getOperator().equals("contains")) {
                    builder.and(
                        timesheetdetails.timeofftype.typename.likeIgnoreCase(
        ...
        for (Map.Entry<String, String> joinCol : joinColumns.entrySet()) {
            ...
            if (joinCol != null && joinCol.getKey().equals("task")) {
                builder.and(timesheetdetails.task.name.eq(joinCol.getValue()));
            }
        }
        for (Map.Entry<String, String> joinCol : joinColumns.entrySet()) {
            ...
            if (joinCol != null && joinCol.getKey().equals("timeofftype")) {
                builder.and(timesheetdetails.timeofftype.typename.eq(joinCol.getValue()));
            }
        }
        for (Map.Entry<String, String> joinCol : joinColumns.entrySet()) {
