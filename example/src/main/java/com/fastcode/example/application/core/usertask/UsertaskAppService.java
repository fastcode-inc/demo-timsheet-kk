...
public class UsertaskAppService implements IUsertaskAppService {
    ...
    public UpdateUsertaskOutput update(UsertaskId usertaskId, UpdateUsertaskInput input) {
        Usertask existing = _usertaskRepository.findById(usertaskId).get();
    ...
    public void delete(UsertaskId usertaskId) {
        Usertask existing = _usertaskRepository.findById(usertaskId).orElse(null);
    ...
    protected BooleanBuilder searchKeyValuePair(
        ...
        for (Map.Entry<String, SearchFields> details : map.entrySet()) {
            ...
            if (details.getKey().replace("%20", "").trim().equals("users")) {
                if (details.getValue().getOperator().equals("contains")) {
                    builder.and(
                        usertask.users.lastname.likeIgnoreCase("%" + details.getValue().getSearchValue() + "%")
                    );
                } else if (details.getValue().getOperator().equals("equals")) {
        ...
        for (Map.Entry<String, String> joinCol : joinColumns.entrySet()) {
            ...
            if (joinCol != null && joinCol.getKey().equals("task")) {
                builder.and(usertask.task.name.eq(joinCol.getValue()));
            }
        }
        for (Map.Entry<String, String> joinCol : joinColumns.entrySet()) {
            ...
            if (joinCol != null && joinCol.getKey().equals("users")) {
                builder.and(usertask.users.lastname.eq(joinCol.getValue()));
            }
        }
        return builder;
    }
